...
int main(int argc, char** argv) {
    ...
    // setup output files
    ofstream myfile, mytimes;
    string filename = "vid_frame";
    string time;
    int seconds = 30;
    int i = 0;
    ...
    //open time file and start timer for times csv
    mytimes.open("times.csv");
...
// Write frame to png file 
        std::string outfile = "/home/pi/Thermal_Images/" + filename + to_string(i) + ".png"; 
        if (_output) 
        ...
        time = to_string(i) + ": " + to_string(elapsed_seconds.count()); 
        filename = "vid_frame"; 
        myfile.close(); 
        mytimes << elapsed_seconds.count() << ","; 
            if (i == (seconds*7)) 
            { 
                mytimes.close(); 
                break;  
